...
class Button_Group(QWidget):
    def __init__(self):
        super().__init__()
        self.setAttribute(Qt.WA_StyledBackground)  # 允许使用样式表设置背景
        # self.setStyleSheet("background-color: rgb(0, 0, 0)")  # 设置背景色
        self.layout = QHBoxLayout(self)
        self.button = Button('Prev', 'primary', ArcoIcon.Left.svg, 'round')
        self.button1 = Button('Next', 'primary', ArcoIcon.Right.svg, 'round', icon_right=True)
        self.button_group = ButtonGroup([self.button, self.button1])
        self.layout.addWidget(self.button_group)

        self.button2 = Button(button_type='primary', icon=ArcoIcon.Start.svg)
        self.button3 = Button(button_type='primary', icon=ArcoIcon.Message.svg)
        self.button4 = Button(button_type='primary', icon=ArcoIcon.Settings.svg)
